...
#Spread each age group's cases by the lognormal distribution
# casedat is a raw csv input file 2D date agegroup cases
#  logcases repopultates with convoluted with lognormal distributed data (propto deaths)
logcases = casedat
...
# comdat 1D array all ages deaths vs cases  Adds one more column called "logcaseload" with convolution with lognormal
...
#   Same again using the gamma distribution.
...
#Calculate age-group CFRs to fit Oct-Nov from Gamma  Regression over period.
# "weight" for each age category converts shifted/read cases to deaths - one number per age group.
#  gampred is predicted deaths WSS/gamma each age group
...
#  Creates all-ages prediction with gamma
...
# "weight" for each age category converts shifted/read cases to deaths - one number per age group.
#  logpred is predicted deaths WSS/lognormal each age group
...
#### Original WSS (hardcoded) Just for presentation in graph ####
...
#### Plot all lognormal-derived CFRs   7 day averages + weird rotation of dataframe for ggplot input ####
