...
##### Does NOT have proper data encapsulation
#####  Figures and analysis for https://www.medrxiv.org/content/10.1101/2021.04.14.21255385v1
  ...
  MedrxivPaper <- function(){ 
    ...
    if(interactive()){
      model = lm(dat, date %in% daterange)$allDeaths ~ filter(gampred, date %in% daterange)$allCasesPred
      summary(model)
      ...
      summary(model)
#  Comparison to David Wallace's code broken by date problems
#    model = lm(filter(comdat, date %in% WSS$date)$allDeaths ~ WSS$values[28:length(WSS$date)])
#      summary(model)
      rm(model)
    }
